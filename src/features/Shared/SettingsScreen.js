            ...
            <ScrollView
                ...
                <Pressable onPress={Keyboard.dismiss} style={tailwind('w-full h-full relative')}>
                    <View style={tailwind('flex flex-row items-center justify-between p-4')}>
                        <Text style={tailwind('text-xl text-gray-50 montserrat-bold')}>{translate('Core.SettingsScreen.title')}</Text>
                    ...
                    <View style={tailwind('flex w-full h-full')}>
                        ...
                        <View style={tailwind('p-4')}>
                            {editableFields !== undefined && editableFields !== null ? (
                                editableFields.map(field => {
                                    return (
                                        <View style={tailwind('mb-4')} key={field}>
                                            <Text style={tailwind('montserrat-bold text-base text-gray-50 mb-2')}>{translate(`Core.SettingsScreen.${field}`)}</Text>
                                            <TextInput
                                ...
                                <Text style={tailwind('montserrat-medium text-base text-gray-50')}>{translate('Core.SettingsScreen.empty')}</Text>
                            )}
                            <TouchableOpacity onPress={saveEntitySettings} disabled={isLoading} style={tailwind('flex')}>
                                <View style={tailwind('btn bg-gray-900 border border-gray-700 mt-6')}>
                                    <Text style={tailwind('montserrat-bold text-lg text-gray-50 text-center')}>{translate('Core.SettingsScreen.save')}</Text>
                                </View>
                            </TouchableOpacity>
                        </View>
                    </View>
