...
const OrganizationSearchScreen = ({ navigation }) => {
    ...
    const renderItem = ({ item }) => (
        <View style={tailwind('p-2')}>
            <TouchableOpacity style={tailwind('p-2 border border-gray-800 shadow-sm ')} onPress={() => handleOrganizationSelection(item)}>
                <View style={tailwind('flex-1 flex-col items-start')}>
                    <Text style={tailwind('text-gray-100 montserrat-medium')}>{item.name}</Text>
                </View>
            </TouchableOpacity>
        </View>
    );
        ...
        <View style={[tailwind('bg-gray-800 flex-1 relative pt-4')]}>
            <View style={tailwind('px-4')}>
                <View style={[searchButtonStyle, tailwind('relative flex-row')]}>
                    ...
                    <TextInput
                        ...
                        textAlign={'left'}
                        style={tailwind('montserrat-medium flex-1 h-full text-white')}
            ...
            {results.length > 0 ? (
                ...
                <View style={tailwind('flex-1 justify-center items-center')}>
                    <Text style={tailwind('montserrat-medium text-gray-400')}>{translate('Auth.SignUpScreen.empty')}</Text>
                </View>
            )}
        </View>
    );
