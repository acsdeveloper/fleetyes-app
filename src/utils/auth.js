...
export default class AuthUtil {
    ...
    /**
 * Hook for retrieving current driver/user ids.
 *
 * @static
 * @return {Array}
 * @memberof AuthUtil
 */
    static userIds() {
        const [value, setValue] = useMMKVStorage('usersIds', storage);
        const fleetbase = useFleetbase();

        const setUserIds = (ids) => {
            console.log('setUserIds() - userId', ids);
            if (typeof ids?.serialize === 'function') {
                emit('ids.updated', ids);
                setValue(ids.serialize());
            } else {
                setValue(ids);
            }
        };
...
const useUserIds = AuthUtil.userIds;
const syncDevice = AuthUtil.syncDevice;
const isValidDriver = AuthUtil.isValid;

export { updateDriver, getDriver, useDriver, syncDevice, isValidDriver, useUserIds };
