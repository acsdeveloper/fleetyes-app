...
import { capitalize, getColorCode, translate } from 'utils';
                        ...
                        {isAndroid ? (
                            <TouchableOpacity onPress={openAndroidDatePicker}>
                                <View style={tailwind('bg-gray-700 rounded-md py-2 px-3')}>
                                    <Text style={tailwind('montserrat-medium text-gray-50')}>{format(date, 'MM/dd/yy')}</Text>
                                </View>
                            </TouchableOpacity>
                        ) : (
                            <DateTimePicker
                    ...
                    <View style={tailwind('pr-2')}>
                        ...
                        >
                            <View style={tailwind('flex flex-row items-center')}>
                                ...
                                <Text style={tailwind(`${sort ? 'text-blue-500' : 'text-gray-300'} montserrat-bold`)}>{translate('terms.sort')}</Text>
                            </View>
                        </TouchableOpacity>
                    </View>
                    <View style={tailwind('pr-2')}>
                        ...
                        >
                            <View style={tailwind('flex flex-row items-center')}>
                                ...
                                <Text style={tailwind(`${!isObjectEmpty(filter) ? 'text-blue-500' : 'text-gray-300'} montserrat-bold`)}>{translate('terms.filter')}</Text>
                            </View>
                        </TouchableOpacity>
                    </View>
                </View>
            ...
            >
                <View>
                    <View style={tailwind('px-5 py-2 flex flex-row items-center justify-between mb-2')}>
                        <View style={tailwind('flex flex-row items-center')}>
                            <Text style={tailwind('text-lg montserrat-bold text-gray-50')}>{capitalize(currentAction)}</Text>
                        </View>
                                    ...
                                    {sortOptions.map((sortOption, index) => (
                                        <TouchableOpacity key={index} style={tailwind('mb-4')} onPress={() => setValue(sortOption.value, 'sort')}>
                                            <View key={index} style={tailwind('btn bg-gray-900 border border-gray-700 rounded-lg shadow-sm justify-start px-4')}>
                                                <Text style={tailwind('text-gray-50 text-lg montserrat-medium')}>{sortOption.label}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    ))}
                                </View>
