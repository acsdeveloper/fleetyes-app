...
import { ActivityIndicator, Dimensions, Modal, Platform, ScrollView, Text, TextInput, TouchableOpacity, View } from 'react-native';
    ...
    return (
        <View style={[wrapperStyle]}>
            <TouchableOpacity onPress={() => setIsDialogOpen(true)}>
                <View style={[tailwind(`flex flex-row items-center bg-gray-800 border border-gray-700 px-3 pr-2 h-10`), buttonStyle]}>
                ...
                <View style={[tailwind('w-full h-full bg-gray-800'), { paddingTop: insets.top }]}>
                        ...
                        <View style={tailwind('flex-1 pr-4')}>
                            <View style={tailwind('relative overflow-hidden')}>
                                ...
                                <TextInput
                                    ...
                                    textAlign={'left'}
                                    style={tailwind('montserrat-medium bg-gray-900 text-white pl-10 shadow-lg pr-2 h-10')}
                    ...
                    {isLoading && (
                        <View style={tailwind('w-full px-5 py-4 flex flex-row items-center')}>
                            <ActivityIndicator />
                            <Text style={tailwind('ml-2 text-gray-400 montserrat-medium')}>{translate('components.interface.NetworkSearch.searching')}</Text>
                        </View>
                    )}
