        ...
        <View style={[tailwind('overflow-hidden'), wrapperStyle]}>
            <View style={[tailwind('w-full'), containerStyle]} onPress={onPress}>
                <View style={tailwind('z-20 relative')}>
                    <View style={[{ height: '100%' }, tailwind(`ml-4 absolute border-l-2 ${isDark ? 'border-white' : 'border-gray-300'} opacity-75`)]} />
                    {payload && (
                        <View style={tailwind('')}>
                            {firstWaypoint && (
                                <View style={tailwind('w-full flex-row items-center mb-4')}>
                                    <View style={tailwind('mr-3')}>
                                        <View style={tailwind(`rounded-full ${isDark ? 'bg-green-custom' : 'bg-green-500'} w-8 h-8 flex items-center justify-center`)}>
                                            <Text style={tailwind('montserrat-bold text-white')}>1</Text>
                                        </View>
                                    </View>
                                    <View style={tailwind('w-4/5')}>
                                        <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'} ${firstWaypoint.completed || isCompleted ? 'line-through' : ''}`), textStyle]}>{firstWaypoint.address}</Text>
                                        {firstWaypoint.phone && (
                                            <TouchableOpacity onPress={() => startCall(firstWaypoint.phone)}>
                                                <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'}`), textStyle]}>{firstWaypoint.phone}</Text>
                                            </TouchableOpacity>
                                        )}
                            ...
                            {isArray(middleWaypoints) &&
                                ...
                                middleWaypoints.map((waypoint, i) => (
                                    <View key={i} style={tailwind('w-full flex-row items-center mb-4')}>
                                        <View style={tailwind('mr-3')}>
                                            <View style={tailwind(`rounded-full ${isDark ? 'bg-green-custom' : 'bg-green-500'} w-8 h-8 flex items-center justify-center`)}>
                                                <Text style={[tailwind('montserrat-bold text-white')]}>{i + 2}</Text>
                                            </View>
                                        </View>
                                        <View style={tailwind('w-full')}>
                                            <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'} ${waypoint.completed || isCompleted ? 'line-through' : ''}`), textStyle]}>{waypoint.address}</Text>
                                            {waypoint.phone && (
                                                <TouchableOpacity onPress={() => startCall(waypoint.phone)}>
                                                    <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'}`), textStyle]}>{waypoint.phone}</Text>
                                                </TouchableOpacity>
                                            )}
                                            {waypoint.tracking_number && (
                                                <View style={tailwind('mt-1 flex flex-row')}>
                                                    <OrderStatusBadge
                                                        ...
                                                        wrapperStyle={[tailwind('flex-grow-0')]}
                                                        style={[tailwind('px-2 py-0.5')]}
                                                        textStyle={[tailwind('text-xs montserrat-medium')]}
                                                    />
                                                </View>
                                            )}
                                        </View>
                                ...
                                <View>
                                    <View style={tailwind('w-full flex-row items-start mb-4')}>
                                        <TouchableOpacity style={tailwind('w-full')} onPress={toggleWaypointCollapse}>
                                            <View style={tailwind(`px-3 py-2 w-full ${isDark ? 'bg-yellow-400' : 'bg-yellow-200'} border border-yellow-500 rounded shadow-sm flex`)}>
                                                <View style={tailwind('flex flex-row')}>
                                                    <FontAwesomeIcon icon={faEye} style={tailwind('mr-2 text-yellow-900')} />
                                                    <Text style={[tailwind('montserrat-bold'), textStyle, tailwind('text-yellow-900')]}>{isWaypointsCollapsed ? 'Tap to expand' : 'Tap to collapse'}</Text>
                                                </View>
                                                <Text style={[tailwind('text-yellow-900 montserrat-medium'), textStyle]}>{middleWaypoints.length} more waypoints</Text>
                                            </View>
                                        </TouchableOpacity>
                                    </View>
                            ...
                            {lastWaypoint && (
                                <View style={tailwind('w-full flex-row items-center')}>
                                    <View style={tailwind('mr-3')}>
                                        <View style={tailwind(`rounded-full ${isDark ? 'bg-red-500' : 'bg-red-600'} w-8 h-8 flex items-center justify-center`)}>
                                            <Text style={tailwind('montserrat-bold text-white')}>{middleWaypoints.length + 2}</Text>
                                        </View>
                                    </View>
                                    <View style={tailwind('w-full')}>
                                        <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'} ${lastWaypoint.completed || isCompleted ? 'line-through' : ''}`), textStyle]}>{lastWaypoint.address}</Text>
                                        {lastWaypoint.phone && (
                                            <TouchableOpacity onPress={() => startCall(lastWaypoint.phone)}>
                                                <Text style={[tailwind(`text-xs montserrat-medium ${isDark ? 'text-gray-50' : 'text-gray-900'}`), textStyle]}>{lastWaypoint.phone}</Text>
                                            </TouchableOpacity>
                                        )}
