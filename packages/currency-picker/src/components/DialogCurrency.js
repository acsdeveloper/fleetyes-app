...
export const DialogCurrency = (props) => {
    ...
    const renderItemTemplate = ({ emoji, code, symbol, name }) => {
        ...
        return (
            <View style={[styles.item, itemContainer]}>
                ...
                <Text style={[styles.isoName, currencyCodeStyle]}>{emoji}</Text>
    ...
    let _flatListRef = useRef<FlatList>(null); // Correct usage of useRef with TypeScript
    ...
    const handleFilterChange = (value) => {
        ...
        if (value === "") {
            ...
            if (_flatListRef.current) _flatListRef.current.scrollToOffset({ offset: 0 });
